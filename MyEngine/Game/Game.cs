...
namespace MyEngine.Game
{
    internal class Game
    {
        ...
        public Game(PictureBox outputBox, Form parentForm)
        {
            ...
            OnStatisticsChanged handler = (stats) => parentForm.Invoke(() => parentForm.Text = stats);
            Bitmap background = new Bitmap(camera.ResolWidth, camera.ResolHeight);
            using (Graphics g = Graphics.FromImage(background))
                g.DrawImage(Properties.Resources.Background, 0, 0, camera.ResolWidth, camera.ResolHeight);
            engine = new Engine.Engine(camera, background, outputBox, handler);
        ...
        private void OnSpheresCollision(SphereModel s1, SphereModel s2)
        {
            ...
            //в будущем можно поиграться с раскраской главной и не только сферbI
        }
